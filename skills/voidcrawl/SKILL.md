...
In Pi, access VoidCrawl through the `mcp` proxy tool and the configured `voidcrawl` server. For web browsing, Google/search-result checks, JS-rendered pages, screenshots, bot-wall probing, and any third-party web content task, prefer VoidCrawl MCP over ad-hoc `bash`/`curl`/Python HTTP fetches. If the current Pi session has not been reloaded since `voidcrawl` was added and `mcp({})` does not list it, say that plainly and ask Andrew to `/reload` or restart instead of silently falling back to raw shell web scraping.

Typical Pi flow:
- discover/list tools: `mcp({ server: "voidcrawl" })` or `mcp({ search: "fetch" })`
- call tools by their prefixed names, e.g. `mcp({ tool: "voidcrawl_fetch", args: '{"url":"https://example.com"}' })`
- use `voidcrawl_fetch`/`voidcrawl_fetch_many` for one-shot pages, `voidcrawl_session_open` + `voidcrawl_session_navigate` for multi-step interaction, and always close sessions when done.
