...
function renderRunRow(run: YosoiRun, width: number, cwd: string): string {
  ...
  const detail = [code, batch, run.fetcher, run.summary, formatDuration(run), formatTokens(run.contextTokens)].filter(Boolean).join(" • ");
...
function visibleRuns(): YosoiRun[] {
  ...
  return runs
    .slice()
    .reverse()
    .slice(dashboardScrollOffset, dashboardScrollOffset + 6);
}
